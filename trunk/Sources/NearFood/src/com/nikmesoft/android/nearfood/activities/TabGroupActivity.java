...
public class TabGroupActivity extends ActivityGroup {
	...
	public void finishFromChild(Activity child) {
		...
		Window window = localActivityManager.destroyActivity(nameActivitys.get(countActivity), true);
		if(window!=null){
			nameActivitys.remove(countActivity);
			countActivity--;
			Activity activityPrevious = localActivityManager.getActivity(nameActivitys.get(countActivity));
			Window windowCurrent = localActivityManager.startActivity(nameActivitys.get(countActivity), activityPrevious.getIntent());
			setContentView(windowCurrent.getDecorView());
		}
	}
	public void startNewActivity(String id, Intent intent){
		Window windowCurrent = localActivityManager.startActivity(id, intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
	...
	public void onBackPressed() {
		...
		Activity topActivity = getLocalActivityManager().getActivity(nameActivitys.get(nameActivitys.size()-1));
		topActivity.finish();
		}
